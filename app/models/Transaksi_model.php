...
class Transaksi_model
{
  ...
  public function tambahDataPemesanan($data)
  {
    ...
    if ($latest) {
      $autoNoTransaksi = $this->autonumber($latest['NoTransaksi'], 2, 5);
    } else {
      $autoNoTransaksi = 'JS00001';
    }

    // TRANSAKSI
    $query = 'INSERT INTO transaksi 
    (NoTransaksi, NIK, Id_Mobil, Tanggal_Pesan, Tanggal_Pinjam, Tanggal_Kembali_Rencana, berangkat, pulang, LamaRental, Id_Sopir, tarifsopir, Mhrg_harian, Total_Bayar, StatusTransaksi, Arsip, tujuan, jaminan, panjar, status_bayar) 
    VALUES(:NoTransaksi, :NIK, :Id_Mobil, :Tanggal_Pesan, :Tanggal_Pinjam, :Tanggal_Kembali_Rencana, :berangkat, :pulang, :LamaRental, :Id_Sopir, :tarifsopir, :Mhrg_harian, :Total_Bayar, :StatusTransaksi, :Arsip, :tujuan, :jaminan, :panjar, :status_bayar)';
	...
	$this->db->bind('tarifsopir', $data['tarifsopir']);
	$this->db->bind('Mhrg_harian', $data['Mhrg_harian']);
	...
	$this->db->bind('jaminan', $data['jaminan']);
	$this->db->bind('panjar', $data['panjar']);
  ...
  public function pesananMobilSelesai($data)
  {
	...
	$sisa_bayar = preg_replace('/\D/', '', $data['sisa_bayar']);
	//$status_bayar = preg_replace('/\D/', '', $data['status_bayar']);
	...
	$this->db->bind('status_bayar', $data['status_bayar']);
	$this->db->bind('sisa_bayar', $sisa_bayar);
